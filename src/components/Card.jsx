...
function Card(props) {
  ...
  return (
    <>
      <div
        ref={cardRef}
        className="text-center mt-4 mb-8 md:mt-12 md:mb-16 focus:outline-none"
        {...getToggleProps({
      ...
      >
        <div
          id={props.id}
          className={`bg-[#C5D4DE] md:w-11/12 md:max-w-2xl mx-auto p-3 md:p-6 rounded-xl border border-[#b3b3b3] shadow-md ${
            expanded ? "pb-8 md:pb-6 active-card" : "pb-4 md:pb-0"
          }`}
        ...
        >
          <div className="flex md:inline-block">
            ...
            <h2 className="md:mb-8 md:-mt-8 my-auto ml-4 md:ml-0 text-left text-[#2B4E6A] font-medium text-3xl md:text-3xl">
              {props.name}
            </h2>
          </div>
        ...
        >
          <button
            className={`rounded-2xl w-20 h-10 bg-[#C5D4DE] rounded-all ${
              expanded
                ? "rotate-180 shadow-none transition-all"
                : "transition-all"
            }`}
          >
            <i
              className={`expand-button mx-auto text-slate-400 fa-duotone fa-angle-up fa-2x ${
                expanded ? "transition-all" : "transition-all"
              }`}
            ></i>
          </button>
        </div>
      ...
      >
        <div className="absolute z-50 -right-2 -top-2 bg-slate-300 shadow-sm w-fit rounded-full p-1">
          <CloseButton onClick={handleAvatarClose} />
        </div>
        <Modal.Body>
          <img
            className="max-w-full h-auto object-cover object-center w-full"
            src={props.img}
            alt="avatar_img"
          />
        </Modal.Body>
        <Modal.Footer>
          <Modal.Title className="text-center text-[#2B4E6A] text-4xl">
            {props.name}
          </Modal.Title>
        </Modal.Footer>
      </Modal>
